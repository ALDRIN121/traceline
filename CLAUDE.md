...
This repository is in **design phase**. There is no source code, no package manifest, no test
suite, and no build system — **so there are no build, lint, test, or run commands.** Do not invent
them, and do not infer them from the design documents: the stack described there is a plan, not an
installed toolchain. Replace this section with real commands when implementation begins.

**The next deliverable is `docs/design/LLM_Agent_Evaluation_Engine_v3.md`, which does not exist
yet.** `docs/design/v2-review-and-v3-plan.md` specifies exactly what it must contain. Writing that
document is the work — not scaffolding code. If asked to "start building," confirm whether v3 is
meant to be written first; the owner's stated sequence was design → review → implementation.
